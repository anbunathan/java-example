...
public class RestMultiFileStoreMongoDBExample {
		...
		if (ServletFileUpload.isMultipartContent(req)) {
			...
			while (iter.hasNext()) {
				...
				if (!item.isFormField()) {	
					...
					//Comment out this line to save with in-built id (objectid)
	...
	@GET
	@Path("/download/object/{id}")
	@Produces(MediaType.APPLICATION_OCTET_STREAM)
	public Response downloadFilebyObjectID(@PathParam("id")  String id) throws IOException {
		...
		query.put("_id", id); 
		GridFS fileStore = new GridFS(mongoDB, "filestore");
		GridFSDBFile gridFile = fileStore.findOne(new ObjectId(id));

		if (gridFile != null && id.equalsIgnoreCase(gridFile.getId().toString())) {
